...
Page({
  ...
  onLoad: function (options) {
    ...
    */
    ...
    this.setData({
      ...
      _target_name: '坚持#' + options.target_name + '#',
      target_title: feed.target,
      target_desc: feed.desc,
      target_image: (feed.images && feed.images.length > 0) ? feed.images[0]["url"] : '',
      nickname: feed.user["nickname"],
      feed_time: feed.show_time,
  ...
  draw: function () {
    ...
    ctx.setFontSize(24)
    ctx.fillText('已坚持' + this.data.target_day + '次', width - 20, 100)

    ctx.setFillStyle('#000000')
    ctx.setFontSize(20)
    ctx.fillText(this.data._target_name, width - 20, 130)
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      title: '我在' + this.data.group.name + '已坚持' + this.data.group.counter + '次#' + this.data.target_name + '#, 邀请你来打卡',
